...
class TwitterOAuth extends Config
{
    ...
    public function url($path, array $parameters)
    {
        ...
        return sprintf('%s/%s/%s?%s', self::API_HOST, self::API_VERSION, $path, $query);
    }
    ...
    public function oauth($path, array $parameters = array())
    {
        ...
        $this->resetLastResponse();
        $this->response->setApiPath($path);
        $url = sprintf('%s/%s/%s', self::API_HOST, self::API_VERSION, $path);
    ...
    private function request($url, $method, $authorization, $postfields)
    {
        ...
        $options = array(
            ...
            //CURLOPT_CAINFO => __DIR__ . DIRECTORY_SEPARATOR . 'cacert.pem', // kaitlyn commented out
            //CURLOPT_CAPATH => __DIR__, // kaitlyn commented out
            CURLOPT_CONNECTTIMEOUT => $this->connectionTimeout,
            CURLOPT_HEADER => true,
            CURLOPT_HTTPHEADER => array('Accept: application/json', $authorization, 'Expect:'),
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_SSL_VERIFYHOST => 2,
            CURLOPT_SSL_VERIFYPEER => false, // kaitlyn changed true to false
